...
```json
{
  "context": {
    "build": {
      "device": "redfin",
      "fingerprint": "google/redfin/redfin:14/UD1A.230803.041/…"
    },
...
- **`device.findObject(...)` returns null** — the test tag is on a debug-only composable, or
  `.testTag(...)` was added inside a `BasicText` whose semantics get merged with the parent. Fix:add
  `Modifier.semantics { testTagsAsResourceId = true }` on the root `LazyColumn` if needed, and
